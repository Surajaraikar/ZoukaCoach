...
export default function Offerings() {
  const offerings = [
    ...
    {
      title: "Safe space to explore and know yourself",
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-6">
          <div>
            ...
            <p className="mt-2 sm:mt-4 text-[#535862] ">
              Our offerings are designed to meet you where you are and gently
              move you forward.
            </p>
          </div>
          <SignedOut>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8 lg:gap-10 mt-12">
          {offerings.map(({ title, desc }) => (
            <div key={title} className="flex items-start gap-4">
              ...
              <div>
                <h3 className="text-lg sm:text-xl font-semibold text-[#553888]">
                  {title}
                </h3>
                <p className="mt-2 text-[#535862] ">{desc}</p>
              </div>
            </div>
          ))}
        </div>
